...
const Metric = ({ icon, value, unit, label, loading }) => (
  <Grid item xs={6} sm={3} sx={{ textAlign: 'center' }}>
    {loading ? <Skeleton width={60} height={40} sx={{ mx: 'auto', bgcolor: 'rgba(255,255,255,0.3)' }} /> : (
      <>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0.5 }}>
            {icon}
            <Typography sx={{ fontWeight: 600 }}>{value}{unit}</Typography>
        </Box>
...
const CurrentWeather = ({ weather, loading, onOpenDetail }) => {
  return (
    <ButtonBase 
      ...
      sx={{ 
          width: '100%', borderRadius: '30px', textAlign: 'left',
    ...
    >
      <Paper elevation={0} sx={{ 
          width: '100%', p: { xs: 2, md: 4 }, borderRadius: '30px', 
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-around', my: 3 }}>
          ...
          <Typography variant="h1" sx={{ fontWeight: 'bold', fontSize: { xs: '5rem', md: '7rem' } }}>
