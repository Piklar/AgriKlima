...
// Import local images for static cards and fallbacks
...
import othersImg from '../assets/images/crop-others.jpg'; // Fallback image
...
const SelectionCard = ({ image, label, isSelected, onClick }) => (
    ...
    >
        <CardMedia component="img" image={image || othersImg} alt={label} sx={{ height: 160, objectFit: 'cover' }} />
...
const SignUpPage = () => {
    ...
    const isStepValid = useMemo(() => {
        switch (step) {
            case 1: return (formData.firstName && formData.lastName && formData.email.includes('@') && formData.mobileNo.length === 11 && formData.password.length >= 8 && formData.password === formData.confirmPassword);
        ...
        try {
            ...
            try {
                ...
                if (profilePictureFile) {
                    ...
                    await api.updateProfilePicture(pictureFormData).catch(e => console.warn("Picture upload failed but continuing...", e));
                }
                            ...
                            return (
                                <Box sx={{ maxWidth: 500, mx: 'auto', textAlign: 'center' }}>
                                    <Stack spacing={3} alignItems="center">
                                        ...
                                        <Typography variant="body2" color="text.secondary">This step is optional. You can add a profile picture later from your profile page.</Typography>
                                    </Stack>
                                </Box>
                            );
                        default:
        ...
        <Box sx={{ backgroundColor: '#f9fafb', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Container maxWidth="md" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', py: 4 }}>
                ...
                <Stack direction="row" justifyContent="center" alignItems="center" spacing={2} sx={{ mt: 4 }}>
                    ...
                    >
                        {isSubmitting ? <CircularProgress size={24} sx={{ color: 'white' }} /> : (step === STEPS.length ? 'Sign Up' : 'Next')}
                    </Button>
                </Stack>
