...
const CropsPage = () => {
  ...
  const handleOpenOverlay = (crop) => {
    setSelectedCrop(crop); // Set the data for the specific crop that was clicked
    setIsOverlayOpen(true); // Open the modal
  };
  ...
  if (loading) return <Typography sx={{ textAlign: 'center', mt: 4 }}>Loading Content...</Typography>;
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={7}>
            {featuredCrop && (
              ...
              >
                <CardMedia component="img" height="350" image={featuredCrop.imageUrl} alt={featuredCrop.name} sx={{ borderRadius: '16px' }} />
                <CardContent sx={{ textAlign: 'center' }}>
                  <Typography variant="h3" component="div" sx={{ fontWeight: 'bold', my: 2 }}>{featuredCrop.name}</Typography>
                  <Typography variant="body1" color="text.secondary">{featuredCrop.description}</Typography>
                </CardContent>
              </Card>
            )}
          </Grid>
          ...
          <Grid item xs={12} md={5}>
            {secondaryCrops.map(crop => (
              <Box key={crop._id} onClick={() => handleOpenOverlay(crop)} sx={{ cursor: 'pointer' }}>
                <SecondaryCropCard title={crop.name} description={crop.description} image={crop.imageUrl} />
              </Box>
            ))}
          </Grid>
        </Grid>
      ...
      <Box sx={{ backgroundColor: '#f0f4e8', py: 6 }}>
        <Container maxWidth="lg">
          <Grid container spacing={5} alignItems="center">
            <Grid item xs={12} md={5}>
              <img src="/assets/images/learn-about-crops.jpg" alt="Learn about crops" style={{ width: '100%', borderRadius: '20px' }} />
            </Grid>
            <Grid item xs={12} md={7}>
