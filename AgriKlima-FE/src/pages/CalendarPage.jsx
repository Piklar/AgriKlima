...
const CalendarPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (user && user.crops) {
        // Create a list matching the sidebar's expected format
      ...
      // Fallback: display static crops if fetch fails
      setUserCrops([
      ...
      setTasks([
        ...
        {
          _id: '2',
          title: 'Check wheat growth progress',
          dueDate: new Date()
        },
        {
          _id: '3',
          title: 'Pest inspection - corn field',
          dueDate: new Date()
        },
        {
          _id: '4',
          title: 'Apply fertilizer to section B',
          dueDate: new Date()
        }
      ]);
    }
  ...
  // Create a list of today's tasks from the fetched tasks
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center' }}>
        <Grid container spacing={4} sx={{ maxWidth: 1200, width: '100%' }}>
            ...
            <Paper elevation={0} sx={{ p: 3, borderRadius: '24px', border: '1px solid #eee', mx: 'auto' }}>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', textAlign: 'center' }}>
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                  <Typography key={day} sx={{ fontWeight: 600, my: 1, color: 'text.secondary', fontSize: '0.875rem' }}>
                    {day}
                  </Typography>
                ))}
                {days.map((day, i) => (
                  ...
                  >
                    ...
                    {/* Example marker, you can add logic to mark days with tasks/crops */}
          ...
          {/* Right Column: Sidebar (NOW DYNAMIC) */}
          <Grid item xs={12} md={4}>
            <SidebarCard title="Current Crops">
              {userCrops.map((crop) => (
                <Paper
                  key={crop.name}
                  elevation={0}
                  sx={{ display: 'flex', alignItems: 'center', p: 1.5, mb: 1, bgcolor: '#fafafa', borderRadius: '16px' }}
                >
                  ...
                  {todayTasks.map((task) => (
                    <ListItem key={task._id || task.title} disableGutters>
                      <ListItemIcon sx={{ minWidth: '32px' }}>
                        ...
                        {task.title && /pest|insect/i.test(task.title) ? (
                          <PestControlOutlinedIcon color="error" />
                        ) : task.title && /fertilizer|apply/i.test(task.title) ? (
                          <AgricultureOutlinedIcon sx={{ color: '#795548' }} />
                        ) : task.title && /check|progress/i.test(task.title) ? (
            ...
            <Paper
              elevation={0}
              sx={{ p: 2.5, borderRadius: '24px', background: '#26c6da', color: 'white' }}
            >
